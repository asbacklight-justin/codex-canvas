...
Codex Canvas 是一个本地优先、可逆的 Codex Desktop 背景与主题管理器。你可以选择本地图片或五款内置原创背景，调整阅读效果、实时预览，并通过临时本地主题通道重新启动 Codex。

**版本 1.0.0** · 联系邮箱：[asbacklight@gmail.com](mailto:asbacklight@gmail.com)
...
从 [GitHub Releases](../../releases/latest) 下载 Universal 压缩包，解压后把 **Codex Canvas.app** 移入 `/Applications`。
...
界面、主题模型、CSS 生成器和 CDP 客户端均按跨平台方式设计。Windows 版本仍需实现安装发现、进程生命周期、启动适配、Authenticode 签名和真机测试，详见[架构说明](docs/ARCHITECTURE.zh-CN.md#windows-路线)。欢迎参与贡献。
